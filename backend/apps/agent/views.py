    ...
    try:
        ...
        if mode == 'assistant':
            ...
            mode_context = "РЕЖИМ АСИСТЕНТА: Ти працюєш в режимі асистента для власника бізнесу. " \
                          ...
                          "\n\nВАЖЛИВО: Ти маєш доступ до функцій календаря. Коли власник просить створити подію/мітінг/запис в календарі - " \
                          "ОБОВ'ЯЗКОВО використовуй функцію book_appointment. НЕ кажи що створив подію якщо не викликав функцію! " \
                          "Для календаря завжди потрібні: назва події (service), дата (date), час (time). " \
